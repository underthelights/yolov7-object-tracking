        ...
        if ret:
            ...
            if bbox_data:
                ...
                for obj in tracked_objects:
                    ...
                    # Draw bounding box with unique ID
                ...
                # Draw pose keypoints
                # for i, kpts in enumerate(kpt_data):
                #     # Draw keypoints without bounding box
                #     for kpt in kpts.reshape(-1, 3):
                #         if kpt[2] > 0:  # Keypoint confidence > 0
                #             x, y = int(kpt[0]), int(kpt[1])
                #             cv2.circle(im0, (x, y), 5, colors(i, True), -1)
                for i, kpts in enumerate(kpt_data):
                    plot_one_box_kpt(xyxy, im0, label=None, color=colors(track_id, True),

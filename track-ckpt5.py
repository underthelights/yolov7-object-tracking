                ...
                if len(pose):
                    for det_index, (*xyxy, conf, cls) in enumerate(reversed(pose[:, :6])):
                        if int(cls) == 0:  # Filter for person class (class 0)
                            ...
                            kpt_data.append(kpts)
            
            if bbox_data:
                bbox_data = np.array(bbox_data)
                tracked_objects = sort_tracker.update(bbox_data)

                print(f"감지된 사람 수: {len(tracked_objects)}")

                for obj in tracked_objects:
                    bbox = obj[:4].astype(int)
                    track_id = int(obj[4])  # Extract the track ID from the object
                    label = f'ID: {track_id}'

                    # Find the corresponding keypoints for this tracked object
                    matched_kpts = None
                    for original_bbox, kpts in zip(bbox_data, kpt_data):
                        if np.all(np.isclose(original_bbox[:4], bbox, atol=1)):
                            matched_kpts = kpts
                            break
                    ...
                    # Draw bounding box with unique ID and keypoints
                    if matched_kpts is not None:
                        plot_one_box_kpt(bbox, im0, label=label, color=colors(track_id, True),
                                        line_thickness=line_thickness, kpt_label=True, kpts=matched_kpts, steps=3,
                ...
                # Display unique IDs and confidence scores
                unique_ids = {int(obj[4]): float(obj[5]) * 100 for obj in tracked_objects}
                print(f"Unique ID: {', '.join([f'ID: {k} (Confidence: {v:.2f}%)' for k, v in unique_ids.items()])}")
